                           ...
                           int noref)
{
    return cfs_hash_bd_lookup_intent(hs, bd, key, hnode,
                    (enum cfs_hash_lookup_intent)(CFS_HS_LOOKUP_IT_ADD |
    ...
    unsigned	  i;
        ...
        cfs_hash_for_each_bd(bds, n, i) {
                ehnode = cfs_hash_bd_lookup_intent(hs, &bds[i], key,
                                                   NULL, (enum cfs_hash_lookup_intent)intent);
                if (ehnode != NULL)
                        return ehnode;
        }
        ...
        struct cfs_hash_ops *ops, unsigned flags)
{
    ...
    LASSERT(ops->hs_put != NULL || ops->hs_put_locked != NULL);
        ...
        hs = (struct cfs_hash *)calloc(1, Offsetof(struct cfs_hash, hs_name[len]));
        if (hs == NULL)
                RETURN(NULL);
...
{
    struct cfs_hash_cond_arg *cond = (struct cfs_hash_cond_arg *)data;
...
cfs_hash_rehash_bd(struct cfs_hash *hs, struct cfs_hash_bd *old)
{
    ...
    cfs_hash_bd_for_each_hlist(hs, old, hhead) {
        hlist_for_each_safe(hnode, pos, hhead) {
             ...
             */
            cfs_hash_bd_from_key(hs, hs->hs_rehash_buckets,
                         hs->hs_rehash_bits, key, &newbd);
            cfs_hash_bd_move_locked(hs, old, &newbd, hnode);
            c++;
        }
    }
    return c;
